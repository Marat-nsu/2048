...
class Game2048:
    ...
    def evaluate_board(self):
        ...
        corner_bonus = WEIGHT_CORNER if max_tile in [self.grid[0,0], 
                                                     self.grid[0,3],
                                                     self.grid[3,0], 
                                                     self.grid[3,3]] else 0
        
        # Smoothness (adjacent differences)
        smoothness = 0
        for i in range(4):
            for j in range(4):
                if j < 3: smoothness += abs(self.grid[i,j] - self.grid[i,j+1])
                if i < 3: smoothness += abs(self.grid[i,j] - self.grid[i+1,j])
        ...
        for i in range(4):
            for j in range(4):
                if j < 3 and self.grid[i,j] == self.grid[i,j+1]: merges += 1
                if i < 3 and self.grid[i,j] == self.grid[i+1,j]: merges += 1
        ...
        return free + corner_bonus + smoothness + merges + ((merges / WEIGHT_MERGES) + 24) * WEIGHT_MONO
